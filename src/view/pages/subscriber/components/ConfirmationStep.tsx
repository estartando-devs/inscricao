...
export const ConfirmationStep = ({
  ...
  <div className="flex flex-col items-center justify-center min-h-[120px] gap-6">
    {selectedCourse && (
      <span className="text-base sm:text-lg font-semibold text-primary-light mb-1 sm:mb-2">
        Curso escolhido:{' '}
        <span className="font-bold">{courses.find((c) => c.value === selectedCourse)?.label}</span>
      </span>
    )}
    <div className="flex flex-col items-center gap-4 w-full mt-4">
      <div className="w-full rounded-xl border border-gray-600 bg-gray-800/80 px-3 py-4 sm:px-5 sm:py-5 text-left text-sm sm:text-base text-gray-200 space-y-3">
        <p className="font-semibold text-primary-light">Sobre renda e vagas</p>
        <p>
          O Estartando Devs foi criado para ampliar oportunidades em tecnologia para pessoas em
          situação de baixa renda. Reservamos também{' '}
          <span className="font-semibold text-gray-100">{amplaConcorrenciaVagasPhrase()}</span> em
          ampla concorrência.
        </p>
        <p className="text-gray-100 font-medium bg-gray-900/60 rounded-lg px-3 py-2 border border-gray-700">
          Critério de renda per capita da família: a soma de todos os rendimentos da família,
          dividida pelo número de moradores da residência, não pode ultrapassar{' '}
          <span className="text-primary-light">{formatLimiteRendaPerCapita()}</span>.
        </p>
        <p className="text-xs sm:text-sm text-gray-400">
          Essa informação é usada no processo seletivo. A declaração falsa pode implicar
          desclassificação, conforme o regulamento do programa.
        </p>
      ...
      <div className="text-center bg-primary-light rounded px-2 py-3 sm:px-4">
        <span className="text-base sm:text-lg font-bold font-family-mono text-gray-800 block">
          Para terminar, uma informação importante: as aulas serão às terças e quintas, de 19h30 às
          21h30. Você tem disponibilidade nesses dias e horário?
        </span>
      </div>
      ...
      <div className="w-full">
        <label
          htmlFor="knownFrom"
          className="text-base font-semibold text-primary-light block mb-2"
        >
      ...
      <div className="flex items-center mt-4 w-full">
        ...
        <label htmlFor="privacy" className="text-sm">
          ...
          <a
            ...
            target="_blank"
            className="underline text-primary-light hover:text-primary-main"
          >
